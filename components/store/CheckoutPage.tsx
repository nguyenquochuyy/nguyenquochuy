      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <form onSubmit={handlePlaceOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-7 space-y-[15px]">
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-between items-center mt-8 gap-[15px]">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-[15px] text-xs text-slate-500 text-center"><span className="flex items-center gap-2"><Lock size={12}/> Bảo mật SSL</span><span className="flex items-center gap-2"><RefreshCw size={12}/> Đổi trả 30 ngày</span><span className="flex items-center gap-2"><ShieldCheck size={12}/> Hàng chính hãng</span></div>
          ...
          <div className="lg:col-span-5">
            <div className="sticky top-28 space-y-[15px]">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <h2 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-3">Tóm tắt đơn hàng ({cart.length})</h2>
                <div className="space-y-[15px] max-h-64 overflow-y-auto pr-2">{cart.map(item => (<div key={item.id + (item.selectedVariantId || '')} className="flex gap-[15px] items-center"><div className="relative w-16 h-16 bg-slate-100 rounded-lg overflow-hidden border border-slate-100 shrink-0"><img src={item.images[0]} className="w-full h-full object-cover"/><span className="absolute -top-2 -right-2 w-5 h-5 bg-slate-800 text-white text-xs font-bold rounded-full flex items-center justify-center border-2 border-white">{item.quantity}</span></div><div className="flex-1 min-w-0"><p className="text-sm font-bold text-slate-800 truncate">{item.name}</p>{item.selectedVariantId && <p className="text-xs text-slate-500">{item.variants.find(v=>v.id === item.selectedVariantId)?.name}</p>}</div><p className="text-sm font-medium text-slate-700">{formatCurrency(calculateItemPrice(item) * item.quantity)}</p></div>))}</div>
